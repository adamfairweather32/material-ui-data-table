...
const getRowAndColumnIdentifiers = id => {
  ...
  if (!id.startsWith(ID_FIELD_PREFIX)) {
    throw Error(
      `id: ${id} is not in expected format. Must start with ${ID_FIELD_PREFIX}`
    );
  }
  const parts = id.split("-");
  return {
    rowIdentifier: parts[1],
    columnIdentifier: parts[2]
  };
};
...
const getIdsForPosition = (row, keys) =>
  keys.reduce((acc, cur) => {
    const { key, columnIndex } = cur;
    return {
      ...acc,
      [columnIndex]: createCellId(row.id, key)
    };
  }, {});
...
export const getGridNavigationMap = (rows = [], columns) => {
  if (!columns || !columns.length) {
    throw Error("No columns provided");
  }
  const includedKeys = getIncludedKeys(columns);
  const hasId = _.every(rows.map(r => r.id), r => !!r || r === 0);
  if (!hasId) {
  ...
  return rows.reduce(
    (acc, row, rowIndex) => ({
      ...
      positionToIdMap: {
        ...acc.positionToIdMap,
        [rowIndex]: getIdsForPosition(row, includedKeys)
      }
    }),
    {}
  );
