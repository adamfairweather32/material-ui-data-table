...
describe("getValidatedRows", () => {
  ...
  it("should throw error when more than one warning rule defined for same field", () => {
    ...
    expect(() => getValidatedRows(rows, rules)).toThrow(
      "Duplicate warning rules detected"
    );
  });
  ...
  it("should throw error when more than one error rule defined for same field", () => {
    ...
    expect(() => getValidatedRows(rows, rules)).toThrow(
      "Duplicate error rules detected"
    );
  });
    ...
    const rules = [
      {
        ...
        getMessage: (value, rows) => {
          return _.sum(rows.map(r => r.foo)) > 20
            ? "Total foo should not be greater than 20"
            : null;
        },
        level: "warn"
      },
