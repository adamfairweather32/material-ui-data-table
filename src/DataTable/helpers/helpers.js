...
const enrich = column => {
  if (
    column.rich &&
    column.rich.autoComplete &&
    column.rich.autoComplete.options
  ) {
  ...
  if (value) {
    if (date && date.format) {
      try {
        ...
        console.error(
          `could not format value: ${value} with format string: ${date.format}`
        );
        return value;
      }
    }
...
export const getDuplicates = (items, keySelector) => {
  const func = i => i;
  return _.keys(
    _.pickBy(_.groupBy(items, keySelector || func), x => x.length > 1)
  );
};
...
export const validateColumns = (columns, reservedColumns = []) => {
  ...
  if (visibleColumns.length) {
    ...
    const withParentHeaderCount = visibleColumns.filter(c => c.parentHeaderName)
      .length;
    if (
      withParentHeaderCount &&
      withParentHeaderCount !== visibleColumns.length
    ) {
      throw new Error(
...
export const translateKeyCodeToChar = keyCode => {
  let mappedKeyCode = isNumPad(keyCode) ? keyCode - 48 : keyCode;
  let char;
...
export const getFormattedCurrencyValue = (
  ...
  if (currencyValue || currencyValue === 0) {
    ...
    if (isNegative) {
      return `(${showCurrencySymbol ? "£" : ""}${formatWithSeparator(
        absolute
      )})`;
    } else {
...
export const filterRow = (row, columns, searchText) => {
  ...
  return _.some(Object.entries(row), ([k, v]) => {
...
export const createCellId = (rowIdentifier, columnIdentifier) =>
  `${ID_FIELD_PREFIX}-${rowIdentifier}-${columnIdentifier}`;
...
export const getColumnType = column => {
  ...
  if (rich) {
    if (
      ...
      rich.autoComplete.options.length
    ) {
      return COMBO_TYPE;
    }
    if (rich.date) {
...
export const cellIsEditing = id => {
  const element = document.getElementById(id);
  const editing =
    element && element.getAttribute(`${DATA_EDITING_PREFIX}-${id}`);
  return editing;
};
...
export const getUpdatedRows = (
  ...
  key,
  rows,
  rowComparator = (r1, r2) => r1.id && r1.id === r2.id
) => {
  if (!rows || !rows.length) {
    return rows;
  }
...
export const getCellIdFromTarget = (
  target,
  depth = DEFAULT_MAX_SEARCH_DEPTH
) => {
  if (!target) {
...
const extractValue = (field, row, columns) => {
  if (field && columns) {
    ...
    if (autoComplete && autoComplete.options) {
      const option = autoComplete.options.find(
